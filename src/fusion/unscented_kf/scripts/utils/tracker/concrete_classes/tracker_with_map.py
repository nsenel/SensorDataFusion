...
class DoubleTrackTrackerMap(AbstractTracker):
    """ Holds 2 seperate track object list one is waiting track list and one valid object list.
        ...
        This tracker has map object where every track object got registered and some additional rules can be applied with map usage.
    """
    def __init__(self, track_generator: AbstractTrackFactory, cost_calculator: AbstractCostCalculator,
                 ...
                 waiting_list_max_hit:int, object_map):
    ...
    def update_waiting_list(self, detections: list, measurment_time: int, measurment_sensor_properties):
        if len(self._waiting_list) == 0:
            for detection in detections:
                key_exist, loc = self._object_map.get_key(detection.measurment_matrix_in_cost_function_cor_system[0], detection.measurment_matrix_in_cost_function_cor_system[1])
                if detection.sensor_type != SensorTypes.radar_cartesian and key_exist and (self._object_map._map[loc]._object_cnt==0 or detection.obj_name !=None):
                    self._waiting_list.append(self.track_generator.generate_track_obj(measurment=detection))
        else:
            ...
            for detection_idx in _un_assigned_detects:
                key_exist, loc = self._object_map.get_key(detections[detection_idx].measurment_matrix_in_cost_function_cor_system[0], detections[detection_idx].measurment_matrix_in_cost_function_cor_system[1])
                if detections[detection_idx].sensor_type != SensorTypes.radar_cartesian and key_exist and (self._object_map._map[loc]._object_cnt==0 or detections[detection_idx].obj_name !=None):   
                    self._waiting_list.append(self.track_generator.generate_track_obj(measurment=detections[detection_idx]))
    ...
    def add_new_tracks(self, un_assigned_detects_measurments: list, measurment_time: int, measurment_sensor_properties) ->None:
        ...
        if len(idxs_obj)>0:
            self._waiting_list[:] = [x for x in self._waiting_list if id(x) not in idxs_obj] # [:] makes inplace change in list dont over write it with new list
        ...
        try:
            ...
            if len(detections)==0:
                    ...
                    self.update_tracks_filter(assigned_tracks=[], un_assigned_tracks=[idx for idx in range(len(in_FOV_tracks_waiting))], detections=[],
                                            tracked_objects=self._waiting_list, measurment_time=measurment_time, time_treshhold_to_remove=self.waiting_list_time_treshhold_to_remove)
